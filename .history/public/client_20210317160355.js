...
function updateUsers(user, remote='') {
    ...
    if(remote!='') {
        usersList.append(
            ...
            .append(
                $('<li>').append($('<button>').attr({act:'Kick', user:user}).addClass('usersettings').text('Kick')).on('click', () => {console.log('click')})
            )
            .append($('<hr>'))
            .append(
                $('<li>').attr({act:'Make-host', user:user}).addClass('usersettings').text('Make Host')
            )
        )
    }
}
