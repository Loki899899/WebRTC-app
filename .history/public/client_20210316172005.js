...
function updateView() {
    ...
    if(Object.keys(peerConnections).length < 3) {
        Object.keys(remoteVidEl).forEach((key) => {
            ...
            Object.keys(remoteVidEl).forEach((key) => {
                remoteVidEl[key].setAttribute('width', window.screen.width/6)
                remoteVidEl[key].setAttribute('height', window.screen.width/6)
            })
    }
}
...
function getLocalStream(message, createAnswer = false) {
    if(navigator.mediaDevices.getUserMedia) {
        ...
        .then((stream) => {
            localStream = stream
            localVideo.setAttribute('width', window.screen.width/1.5)
            localVideo.setAttribute('height', window.screen.width/1.5)
