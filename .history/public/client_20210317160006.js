...
function updateUsers(user, remote='') {
    ...
    if(remote!='') {
        usersList.append(
            ...
            .append(
                $('<li>').append($('<button>').attr({act:'Kick', user:user}).addClass('user-settings').text('Kick'))
            )
            .append($('<hr>'))
            .append(
                $('<li>').attr({act:'Make-host', user:user}).addClass('user-settings').text('Make Host')
            )
        )
    }
}
